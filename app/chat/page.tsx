...
export default function ChatPage() {
  ...
  const loadData = async () => {
    ...
    if (profile) {
    // Студенты не могут заходить в чат
...
const sendMessage = async () => {
  ...
  // Только для родителей проверяем AI
  if (currentUser.role === 'parent') {
    ...
    if (!isSafe) {
      ...
      await supabase.from('moderation_logs').insert({
        ...
        ai_reason: 'AI moderation blocked'
      })
      setAiWarning(true)
  ...
  const roleColors: Record<'teacher' | 'parent' | 'student', { bg: string; text: string }> = {
